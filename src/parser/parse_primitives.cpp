...
void parseSpheres(const libconfig::Setting &primitives, Core *core)
{
    ...
    for (int i = 0; i < spheres.getLength(); ++i) {
        ...
        core->addPrimitive(std::make_unique<RayTracer::Sphere>(center, radius, RayTracer::RGB{r, g, b}));
    }
}
...
void parsePlanes(const libconfig::Setting &primitives, Core *core)
{
    ...
    for (int i = 0; i < planes.getLength(); ++i) {
        ...
        int g = static_cast<int>(color["g"]);
        int b = static_cast<int>(color["b"]);
        core->addPrimitive(std::make_unique<RayTracer::Plane>(-1 * position, axis, RayTracer::RGB{r, g, b}));
    }
}
...
void parseCones(const libconfig::Setting &primitives, Core *core)
{
    ...
    for (int i = 0; i < cones.getLength(); ++i) {
        ...
        int g = static_cast<int>(color["g"]);
        int b = static_cast<int>(color["b"]);
        core->addPrimitive(std::make_unique<RayTracer::Cone>(basePoint, headPoint, radius, RayTracer::RGB{r, g, b}));
    }
}

void parseCylindres(const libconfig::Setting &primitives, Core *core)
{
    if (!primitives.exists("cylindres"))
        return;
    ...
    for (int i = 0; i < cylindres.getLength(); ++i) {
        ...
        int g = static_cast<int>(color["g"]);
        int b = static_cast<int>(color["b"]);
        core->addPrimitive(std::make_unique<RayTracer::Cylander>(basePoint, height, radius, RayTracer::RGB{r, g, b}));
    }
}
